...
let BatchService = class BatchService {
    ...
    async getAllBatch({ inicio, pageSize }) {
    ...
    async createBatch(batchDTO) {
        return await this.entity.save(batchDTO);
    }
    async updateBatch(id, batchDTO) {
        const task = await this.entity.findOne({ where: { id: id } });
        if (task) {
            this.entity.merge(task, batchDTO);
...
BatchService = __decorate([
    ...
    __param(2, (0, nestjs_prometheus_1.InjectMetric)('batch_served')),
