...
let MunicipalityService = class MunicipalityService {
    ...
    async getAllMunicipality({ inicio, pageSize }) {
    ...
    async createMunicipality(municipalityDTO) {
        return await this.entity.save(municipalityDTO);
    }
    async updateMunicipality(id, municipalityDTO) {
        const task = await this.entity.findOne({ where: { id: id } });
        if (task) {
            this.entity.merge(task, municipalityDTO);
