...
let LegalSupportsService = class LegalSupportsService {
    ...
    async getAllLegalSupports({ inicio, pageSize }) {
    ...
    async createLegalSupports(legalSupportsDTO) {
        return await this.entity.save(legalSupportsDTO);
    }
    async updateLegalSupports(id, legalSupportsDTO) {
        const task = await this.entity.findOne({ where: { id: id } });
        if (task) {
            this.entity.merge(task, legalSupportsDTO);
...
LegalSupportsService = __decorate([
    ...
    __param(0, (0, typeorm_1.InjectRepository)(legal_supports_entity_1.LegalSupportsEntity)),
    __param(1, (0, common_1.Inject)(nest_winston_1.WINSTON_MODULE_PROVIDER)),
    __param(2, (0, nestjs_prometheus_1.InjectMetric)('legal_supports_served')),
