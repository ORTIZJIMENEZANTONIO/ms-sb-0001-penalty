...
let DelegationService = class DelegationService {
    ...
    async getAllDelegation({ inicio, pageSize }) {
    ...
    async createDelegation(delegationDTO) {
        return await this.entity.save(delegationDTO);
    }
    async updateDelegation(id, delegationDTO) {
        const task = await this.entity.findOne({ where: { id: id } });
        if (task) {
            this.entity.merge(task, delegationDTO);
...
DelegationService = __decorate([
    ...
    __param(0, (0, typeorm_1.InjectRepository)(delegation_entity_1.DelegationEntity)),
    __param(1, (0, common_1.Inject)(nest_winston_1.WINSTON_MODULE_PROVIDER)),
    __param(2, (0, nestjs_prometheus_1.InjectMetric)('delegation_served')),
