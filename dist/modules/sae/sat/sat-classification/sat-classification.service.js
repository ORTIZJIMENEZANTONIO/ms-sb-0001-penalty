...
let SatClassificationService = class SatClassificationService {
    ...
    async getAllSatClassification({ inicio, pageSize }) {
    ...
    async createSatClassification(satClassificationDTO) {
        return await this.entity.save(satClassificationDTO);
    }
    async updateSatClassification(id, satClassificationDTO) {
        const task = await this.entity.findOne({ where: { id: id } });
        if (task) {
            this.entity.merge(task, satClassificationDTO);
...
SatClassificationService = __decorate([
    ...
    __param(0, (0, typeorm_1.InjectRepository)(sat_classification_entity_1.SatClassificationEntity)),
    __param(1, (0, common_1.Inject)(nest_winston_1.WINSTON_MODULE_PROVIDER)),
    __param(2, (0, nestjs_prometheus_1.InjectMetric)('sat_classification_served')),
